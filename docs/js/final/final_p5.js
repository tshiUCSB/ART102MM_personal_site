...
function tool_click_handler(e) {
	...
	this.style.opacity = clicked_tool.selected ? "1" : ".8";
...
function draw() {
	if (states.mic_on) {
		if (states.draw_started) {
			...
			console.log(dy)
			// dy = constrain(dy, 0, 500);
			// dy = map(dy, 0, 500,
			// 	parseInt(sliders.max_pencil_pitch.min), 
			// 	parseInt(sliders.max_pencil_pitch.value));
			...
			dy = map(dy, pitch_min, pitch_max, -50, 50);
			...
			draw_position.x += parseInt(sliders.x_speed.value);
			draw_position.y += dy;
		}
	}
}
